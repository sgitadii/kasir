        ...
        <form action="{{ route('customers.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="alamat" name="alamat" rows="4" placeholder="Masukkan alamat pelanggan"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
                @error('alamat')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
